...
const ResidentList = ({ residents }) => {
  ...
  paginationLogic(currentPage, residents);
  ...
  <section className="max-w-[1000px] mx-auto">
    ...
    <ul className="flex justify-center p-4 gap-6 flex-wrap">
      {pages.map((page) => (
        <li key={page}>
          <button className={`$"currentPage === page ? "bg-green-800 p-2 text-white rounded-md" : " bg-green-800 rounded-md text-white"`} onClick={() => handleNewPage(page)}>
            {page}
          </button>
        </li>
        ))}
